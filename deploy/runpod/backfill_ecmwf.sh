...
#
# Usage: bash deploy/runpod/backfill_ecmwf.sh START END [N_WORKERS=4]
# e.g.:  nohup bash deploy/runpod/backfill_ecmwf.sh 2023-02-01 2023-02-28 4 \
...
for ((k = 0; k < WORKERS; k++)); do
  (
    for ((i = k; i < ${#ALL[@]}; i += WORKERS)); do
      d=${ALL[i]}
      uv run pep backfill-ecmwf --start "$d" --end "$d" || echo "FAIL $d"
    done
    echo "worker $k done"
